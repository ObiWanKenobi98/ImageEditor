...
{
    public class SepiaImageFilter : ImageFilter
    {
        public Image applyFilter(Image image, double value)
        {
            ...
            Parallel.For(0, imageBytes.Length / 3, (index) =>
            {
                ...
                imageBytes[index * 3] = (byte)Math.Max(Math.Min((1 - value) * imageBytes[index * 3] + value * blueFinalValue, 255), 0);
                imageBytes[index * 3 + 1] = (byte)Math.Max(Math.Min((1 - value) * imageBytes[index * 3 + 1] + value * greenFinalValue, 255), 0);
                imageBytes[index * 3 + 2] = (byte)Math.Max(Math.Min((1 - value) * imageBytes[index * 3 + 2] + value * redFinalValue, 255), 0);
            });
