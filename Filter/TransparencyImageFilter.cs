...
{
    public class TransparencyImageFilter : ImageFilter
    {
        public Image applyFilter(Image image, double value)
        {
            ...
            Parallel.For(0, imageBytes.Length / 4, (index) =>
            {
                ...
                imageBytes[index * 4] = (byte)Math.Max(Math.Min((1 - value) * imageBytes[index * 4] + value * alphaComputedValue, 255), 0);
                imageBytes[index * 4 + 1] = (byte)Math.Max(Math.Min((1 - value) * imageBytes[index * 4 + 1] + value * alphaComputedValue, 255), 0);
                imageBytes[index * 4 + 2] = (byte)Math.Max(Math.Min((1 - value) * imageBytes[index * 4 + 2] + value * alphaComputedValue, 255), 0);
            });
